...
export default <TSchema extends Record<string, unknown>>({ postgres }: { postgres: Postgres<TSchema> }) => {
    ...
    router.post('/', async (req: WebhookRequest, res: Response, next: NextFunction) => {
        ...
        switch (event_type) {
            case EventType.PLAN_CREATED: return await onCreated(content.plan, res);
            case EventType.PLAN_UPDATED: return await onUpdated(content.plan, res);
            case EventType.PLAN_DELETED: return await onDeleted(content.plan, res);
            default: return next()
        }
    })
