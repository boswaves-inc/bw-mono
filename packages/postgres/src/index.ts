...
export class Postgres<Schema extends Record<string, unknown>> {
    private _store: PostgresJsDatabase<Schema>

    public constructor({ schema, config, logger }: { schema: Schema, config: postgres.Options<{}> | undefined, logger: Logger }) {
    ...
    public transaction<T>(transaction: (tx: PgTransaction<PostgresJsQueryResultHKT, Schema, ExtractTablesWithRelations<Schema>>) => Promise<T>, config?: PgTransactionConfig): Promise<T> {
