...
export class Postgres<TSchema extends Record<string, unknown> = Record<string, never>> {
    // constructor() {
    ...
    private get _store() {
        ...
        return store as PostgresJsDatabase<TSchema>
    }
    ...
    public transaction<T>(transaction: (tx: PgTransaction<PostgresJsQueryResultHKT, TSchema, ExtractTablesWithRelations<TSchema>>) => Promise<T>, config?: PgTransactionConfig): Promise<T> {
