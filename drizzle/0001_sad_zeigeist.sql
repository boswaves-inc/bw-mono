...
CREATE UNIQUE INDEX "item_info_slug_unq" ON "item" USING btree ("slug");--> statement-breakpoint
...
CREATE UNIQUE INDEX "item_price_item_idx" ON "item_price" USING btree ("item_id");--> statement-breakpoint
CREATE UNIQUE INDEX "item_price_name_unq" ON "item_price" USING btree ("name") WHERE status != 'deleted';--> statement-breakpoint
CREATE UNIQUE INDEX "item_price_item_currency_unq" ON "item_price" USING btree ("item_id","currency_code","period","period_unit") WHERE status != 'deleted';--> statement-breakpoint
...
CREATE VIEW "public"."cart_data" AS (select "cart_info"."id", "cart_info"."uid", json_agg(json_build_object('id', "item"."id", 'name', "item"."name", 'type', "item"."type", 'slug', "item"."slug", 'status', "item"."status", 'created_at', "item"."created_at", 'updated_at', "item"."updated_at")) as "items" from "cart_info" left join "cart_item" on "cart_info"."id" = "cart_item"."id" inner join "item" on "cart_item"."item" = "item"."id" group by "cart_info"."id");