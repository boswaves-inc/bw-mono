    ...
    return (
        <div className={cn("markdown", className)}>
            <Primitive
                ...
                components={{ p, h1, h2, h3, blockquote, img, hr, ol, ul, li, strong, link, a, code, pre, table, th, td }}
            >
                {content}
            </Primitive>
        </div>
...
const h1 = ({ children, node, ...props }: ComponentProps<'h2'> & ExtraProps) => createElement('h2', {
    ...props,
    className: 'mt-12! font-normal! mb-10! text-4xl! tracking-tight! text-gray-950! first:mt-0! last:mb-0!'
}, children)
    ...
    return (
        <span className="relative! overflow-hidden! inline-flex! flex-col! items-start! rounded-3xl! border! border-gray-300! bg-gray-50! text-sm/6! font-medium! text-gray-500!">
            ...
            <span className='overflow-y-auto! overflow-x-auto! px-2! h-fit! w-full! relative! '>
                <code className={cn(className, "whitespace-pre! py-0!")}>
                    {children}
                </code>
            </span>
        </span>
